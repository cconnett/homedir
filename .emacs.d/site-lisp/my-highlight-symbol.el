...
;; TODO(cjc): This should be customize-able.
...
(defun get-color-for-symbol (symbol)
...
element in of `highlight-symbol-faces'."
  ...
  (let ((symbol (or symbol
    ...
    (if (highlight-symbol-symbol-highlighted-p symbol)
      ...
      (highlight-symbol-add-symbol symbol (get-color-for-symbol symbol))
      (when (member 'explicit highlight-symbol-occurrence-message)
        (highlight-symbol-count symbol t)))))
;; TODO(cjc): I had implemented a common highlight list for all buffers. Ensure that works again.
...
;; TODO(cjc): Ensure this works over all buffers again.
...
(defun highlight-symbol-next ()
  ...
  (interactive)
  (highlight-symbol-jump-from-point 1))
...
(defun highlight-symbol-next-force ()
  ...
  (interactive)
  (highlight-symbol-jump-from-save 1))
...
(defun highlight-symbol-get-symbol ()
  ...
  (let ((symbol (thing-at-point 'symbol)))
    (when (and symbol
               (not (or (mapcar (lambda (e) (string-match e symbol))
...
(defun highlight-symbol-jump (symbol dir &optional offset)
...
DIR has to be 1 or -1."
  (let* ((case-fold-search nil)
    ...
    ;; move past the end bound of the symbol so we don't find the same instance again
    ;; (goto-char (if (< 0 dir) (cdr bounds) (car bounds)))
    (let* ((start (point))
           (target (re-search-forward symbol nil t dir)))
      (when (equal start (match-beginning 0))
...
(defun highlight-symbol-jump-from-point (dir)
  (let ((symbol (highlight-symbol-get-symbol)))
    (if symbol
        (let* ((bounds (bounds-of-thing-at-point 'symbol))
               (offset (- (point) (car bounds))))
          (highlight-symbol-jump symbol dir offset)
          )
      (error "No symbol at point")
      )))

(defun highlight-symbol-jump-from-save (dir)
  (let* ((symbol (or highlight-symbol-last-symbol
                     (highlight-symbol-get-symbol)
                     (error "No symbol at point")))
         (bounds (bounds-of-thing-at-point 'symbol)))
    (highlight-symbol-jump symbol dir (if bounds (- (point) (car bounds)) 0)))
  )
